...
2. The provider routing cascade correctly selects the provider for at least 10 model names covering all routing rules (e.g., `claude-sonnet-4-6` → Anthropic, `grok-3` → xAI, `openai/gpt-4o` → OpenAI-compat, `gpt-4o` → OpenAI-compat, `qwen-max` → DashScope, `kimi-k2.5` → DashScope, `local/llama3` → OpenAI-compat, an unknown model with only `ANTHROPIC_API_KEY` set → Anthropic, an unknown model with only `OPENAI_API_KEY` set → OpenAI-compat, an unknown model with no credentials → Anthropic default).
...
4. The SSE stream parser correctly processes a synthetic event stream containing all six event types (SessionStart, ContentBlockBegin, ContentBlockDelta, ContentBlockEnd, MessageDelta, SessionEnd) and produces a complete `MessageResponse`. Test cases must include: (a) a clean complete stream, (b) a stream with multiple content blocks, (c) a stream interrupted after ContentBlockDelta (partial response preserved), (d) a stream with an unknown event type (skipped gracefully).
5. Configuration loading discovers, reads, and merges all five config file levels, with later files correctly overriding earlier files for overlapping keys while preserving non-overlapping keys. Test: create config files at levels 1, 3, and 5 with overlapping `ModelAliases` keys; verify that level 5's value wins and level 1's non-overlapping keys are preserved.
6. Model alias resolution maps all eight built-in aliases to their correct model identifiers and correctly applies user-defined overrides from config files. Test: resolve `opus` → `claude-opus-4-7`, then add a config override `opus` → `custom-opus-v2` and verify the override takes effect.
7. Workspace path scope validation correctly allows paths within the workspace root, denies paths outside the workspace root (including symlink escapes), handles glob expansion, environment variable expansion, home directory shorthand, shell redirections, and Windows paths per §3.6 and §4.7. Test vectors: (a) `./src/main.rs` → allowed, (b) `/etc/passwd` → denied, (c) symlink from workspace to `/tmp/escape` → denied, (d) `~/file` with `$HOME` outside workspace → denied, (e) `>output.txt` → `output.txt` extracted and validated, (f) malformed quotes `"unterminated` → fallback to whitespace split.
8. The usage tracker correctly accumulates token counts across multiple `record()` calls, computes cost using the model-specific pricing table (including cache write and cache read rates), and formats costs as `$<amount>` with 4 decimal places. Test: record 1000 input + 500 output + 200 cache_write + 100 cache_read tokens for a Haiku-class model; verify cost = `$0.0038` (1000×1.00 + 500×5.00 + 200×1.25 + 100×0.10) / 1,000,000.
9. The usage tracker reconstruction function produces a tracker state identical to the original tracker state when given the same sequence of usage-bearing messages. Test: simulate 5 turns of usage records, serialize the messages, reconstruct from messages, and assert cumulative totals and turn count match.
10. The POSIX shell tokenizer correctly parses quoted strings, handles escaped characters, and falls back to whitespace splitting when encountering malformed shell syntax. Test vectors: (a) `echo 'hello world'` → two tokens, (b) `echo "hello \"world\""` → two tokens, (c) `echo 'unterminated` → fallback produces two tokens, (d) `VAR=value cmd` → `VAR=value` filtered as env assignment.
